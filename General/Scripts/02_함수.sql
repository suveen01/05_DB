 ...
 * 
 *  2) 소수점 위치 O : 지정된 위치에서 표현 되도록 반올림
 * 
...
-- TRUNC()와 FLOOR()의 차이(양수는 확인이 안되고 음수에서 확인 가능)
-- 버림			 내림

SELECT 
	TRUNC(123.5), FLOOR(123.5), -- 123
...
SELECT 
	TO_CHAR(SYSDATE, 
	'YYYY"년" MM"월" DD"일" (DY) PM HH"시" MI"분" SS"초"' ) AS "오늘 날짜" 
FROM DUAL;
...
SELECT 
	TO_CHAR(SYSDATE, 
	'RRRR"년" MM"월" DD"일" (DY) PM HH"시" MI"분" SS"초"' ) AS "오늘 날짜" 
FROM DUAL;
