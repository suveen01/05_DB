 ...
 * 
 * - DATE타입 : 년,월,일,시,분,초,요일 저장
 * - TIMESTAMP 타입 : 년,월,일,시,분,초,요일,ms,지역시간 저장
 * 
 ...
 * 
 * EX) 문자열 		 오라클 인식 
 * 			abc  ->  ABC,abc(대소문자 구문 X)
 * 		 "abc" ->   a,b,c	("" 내부 작성된 모양으로만 인식)
 * 
 * 
 *  */
...
SELECT
	...
	FLOOR(CURRENT_DATE-HIRE_DATE) AS 근무일수, 
 ...
 * - CHAR,VARCHAR 타입(문자열): 'AB','가나다'('' 홑따옴표)
 * 
 * * SELECT절에 리터럴을 작성하면 
 * 	 조회 결과(RESULT SET) 모든 행에 리터럴이 추가됨
 */
